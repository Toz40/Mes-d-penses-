...
function openExpense(withdrawalId='',expenseId=''){
 ...
 const selectedCurrency=forcedCurrency||(existing?.currency||'EUR');
...
function openWithdrawalDetails(id){
 ...
 closeOnlyModal(`<h2>${esc(w.title||'Retrait espèces')}</h2><div class="small muted">${w.date} · ${esc(participantName(w.ownerId))}</div><div class="grid" style="margin-top:12px"><div class="stat"><span class="small muted">Montant</span><strong>${fmt(w.amount,cur)}</strong></div><div class="stat"><span class="small muted">Restant</span><strong>${fmt(rem,cur)}</strong></div></div><h3 style="margin-top:18px">Détail des dépenses</h3>${expenses.map(e=>`${expRow(e)}<div class="btnrow" style="justify-content:flex-end"><button type="button" class="secondary small" data-detail-edit-exp="${e.id}">Modifier</button></div>`).join('')||'<div class="empty">Aucune dépense liée à ce retrait.</div>'}<div class="modal-actions"><button type="button" class="secondary" id="detailAddExpense">+ Dépense</button><button type="button" class="secondary" id="detailEditWithdrawal">Modifier le retrait</button><button type="button" class="ghost" id="cancelModal">Fermer</button></div>`);
