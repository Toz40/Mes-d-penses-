...
function expensesView(){const es=group().expenses.slice().sort((a,b)=>b.date.localeCompare(a.date));return`<div class="btnrow"><button class="primary" data-action="expense">+ Ajouter</button></div><section class="card"><h3>Toutes les dépenses</h3>${es.map(e=>`${expenseDisplayRow(e,{showPayment:true})}<div class="btnrow" style="justify-content:flex-end"><button class="secondary small" data-edit-exp="${e.id}">Modifier</button><button class="danger small" data-delete-exp="${e.id}">Supprimer</button></div>`).join('')||'<div class="empty">Aucune dépense enregistrée</div>'}</section>`}
...
function balancesView(){const b=balances(),ds=debts(),groups=summaryGroups(balanceSummary);return`<section class="card"><h3>Solde par participant</h3>${group().participants.map(p=>`<div class="row"><span>${esc(p.name)}</span><span class="amount">${fmt(b[p.id],'EUR')}</span></div>`).join('')}</section><section class="card"><h3>Remboursements suggérés</h3>${ds.map(d=>`<div class="row"><span><strong>${esc(participantName(d.from))}</strong> rembourse <strong>${esc(participantName(d.to))}</strong></span><span class="amount">${fmt(d.amount,'EUR')}</span></div>`).join('')||'<div class="empty">Tout est équilibré</div>'}</section><section class="card"><h3>Récapitulatif des dépenses</h3><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px"><button class="${balanceSummary==='category'?'primary':'secondary'} small" data-summary="category" style="padding-left:6px;padding-right:6px">Par catégorie</button><button class="${balanceSummary==='payer'?'primary':'secondary'} small" data-summary="payer" style="padding-left:6px;padding-right:6px">Par payeur</button><button class="${balanceSummary==='participant'?'primary':'secondary'} small" data-summary="participant" style="padding-left:6px;padding-right:6px">Par participant</button></div>${groups.map(gr=>`<div style="display:flex;justify-content:space-between;gap:12px;margin:16px 0 6px;font-weight:800;font-size:17px;color:#d51f2b"><span>${esc(gr.title)}</span><span style="white-space:nowrap">${fmt(summaryTotalEur(gr),'EUR')}</span></div>${gr.items.map(x=>summaryExpenseLine(x.e,x.amount)).join('')}`).join('')||'<div class="empty">Aucune dépense</div>'}</section>`}
...
function bindDynamic(){
 ...
 document.querySelectorAll('[data-summary]').forEach(x=>x.onclick=()=>{balanceSummary=x.dataset.summary;render()});
}
