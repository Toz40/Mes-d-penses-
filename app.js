...
function withdrawalDetailExpenseRow(e){
 const c=categoryObj(e.category),cur=e.currency||'EUR',splits=expenseSplitsNative(e);
 const splitText=Object.entries(splits).filter(([,v])=>Math.abs(Number(v||0))>.0001).map(([pid,v])=>`${esc(participantName(pid))} : ${fmt(v,cur)}`).join(' · ')||'Aucune ventilation';
 return`<div class="row" style="align-items:flex-start"><div style="min-width:0;flex:1"><div style="display:flex;align-items:center;gap:8px"><span style="display:inline-flex;width:34px;height:34px;border-radius:10px;background:${esc(c.color)}18;align-items:center;justify-content:center;flex:0 0 auto"><img src="${esc(c.icon)}" alt="" style="width:22px;height:22px"></span><strong style="font-size:18px">${esc(e.title)}</strong></div><div class="small muted" style="margin-top:7px"><strong>Catégorie :</strong> ${esc(e.category)}</div><div class="small muted" style="margin-top:4px;line-height:1.5"><strong>Ventilation :</strong> ${splitText}</div></div><div class="amount" style="text-align:right;margin-left:10px"><div style="font-size:18px">${fmt(e.amount,cur)}</div><div class="small muted" style="margin-top:4px">≈ ${fmt(eurAmount(e),'EUR')}</div></div></div>`
}
...
function bindDynamic(){
 ...
 document.querySelectorAll('[data-action="participant"]').forEach(x=>x.onclick=openParticipant);
...
function openWithdrawalDetails(id){
 ...
 closeOnlyModal(`<h2>${esc(w.title||'Retrait espèces')}</h2><div class="small muted">${w.date} · ${esc(participantName(w.ownerId))}</div><div class="grid" style="margin-top:12px"><div class="stat"><span class="small muted">Montant</span><strong>${fmt(w.amount,cur)}</strong></div><div class="stat"><span class="small muted">Restant</span><strong>${fmt(rem,cur)}</strong></div></div><h3 style="margin-top:18px">Détail des dépenses</h3>${expenses.map(e=>`${withdrawalDetailExpenseRow(e)}<div class="btnrow" style="justify-content:flex-end"><button type="button" class="secondary small" data-detail-edit-exp="${e.id}">Modifier</button></div>`).join('')||'<div class="empty">Aucune dépense liée à ce retrait.</div>'}<div class="modal-actions"><button type="button" class="secondary" id="detailAddExpense">+ Dépense</button><button type="button" class="secondary" id="detailEditWithdrawal">Modifier le retrait</button><button type="button" class="ghost" id="cancelModal">Fermer</button></div>`);
