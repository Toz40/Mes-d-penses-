...
function openExpense(withdrawalId='',expenseId=''){
 ...
 setTimeout(()=>{
   ...
   document.getElementById('cancelModal').onclick=()=>document.getElementById('modal').close();const saveBtn=document.getElementById('saveExpenseBtn'),form=document.getElementById('modalForm'),hiddenSubmit=document.getElementById('expenseHiddenSubmit');if(saveBtn&&form)saveBtn.onclick=()=>{if(typeof form.requestSubmit==='function'){form.requestSubmit(hiddenSubmit||undefined)}else if(hiddenSubmit){hiddenSubmit.click()}else{const evt=document.createEvent('Event');evt.initEvent('submit',true,true);form.dispatchEvent(evt)}};document.getElementById('splitMode').onchange=e=>document.getElementById('customSplit').classList.toggle('hidden',e.target.value!=='custom');
